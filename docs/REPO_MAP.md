...
| `scripts/models_capture_1_1_12_runner.py` | Independent explicit-account `models` capture bridge. It runs one bounded snapshot-executable process with source logical argv and closed environment, records only private `captured` evidence, and cannot accept inventory, update metadata, route, retry, or inspect HOME contents. Unrelated capture-parent siblings stay outside authority; the one owned root is descriptor/path bound and has an exact internal inventory before the child starts. | `tests/test-models-capture-1-1-12-runner.py` (51 offline cases) |
