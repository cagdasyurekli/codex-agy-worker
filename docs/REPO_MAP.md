...
| `scripts/version_attestation_runner.py` | Canonical fixed-profile snapshot-backed `--version` attestation; isolated system-Python startup, prior-root/snapshot/source authority binding, one exact Popen, bounded streams/pre-reap group cleanup, private durable binding, and a synthetic-only self-test path. Production execution remains a separate explicit action. | `tests/test-version-attestation-runner.py` (76 cases) |
