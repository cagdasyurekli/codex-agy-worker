...
| `scripts/models_capture_1_1_12_profile.py` | Independent process-inert bridge from exact recovered 1.1.12 source/snapshot/binding evidence to a canonical profile with explicit capture parent. It never lists or reads account HOME, launches a child, uses provider/network/Git authority, or authorizes capture. | `tests/test-models-capture-1-1-12-profile.py` (28 offline cases) |
| `scripts/models_capture_1_1_12_runner.py` | Independent explicit-account `models` capture bridge. It runs one bounded snapshot-executable process with source logical argv and closed environment, records only private `captured` evidence, and cannot accept inventory, update metadata, route, retry, or inspect HOME contents. | `tests/test-models-capture-1-1-12-runner.py` (48 offline cases) |
