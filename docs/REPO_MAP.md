...
| `scripts/version_attestation_runner.py` | Canonical fixed-profile snapshot-backed `--version` attestation; isolated root-owned/non-writable Apple system-interpreter startup, bounded path-redacted rejection diagnostics, prior-root/snapshot/source authority binding, one exact Popen, bounded streams/pre-reap group cleanup, private durable binding, and a synthetic-only self-test path. Diagnostic output is drift evidence, not acceptance. Production execution remains a separate explicit action. | `tests/test-version-attestation-runner.py` (136 cases) |
...
| `.github/workflows/test.yml` | macOS CI for syntax and all ten offline suites | exercised by GitHub Actions |
