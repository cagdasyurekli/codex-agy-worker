#!/usr/bin/env python3
"""Sidecar maintenance tool for classifying manifest-bound capture failures.
...
CLASSIFIER_PROG = "models_capture_1_1_22_classifier.py"
...
def classify_evidence_root(
    ...
    try:
        ...
        runner_sha_file_bytes, _ = _read_file_at(dir_fd, RUNNER_ARTIFACT_NAME + ".sha256", 128, 0o600)
        expected_runner_sha_text = (EXPECTED_RUNNER_SHA256 + "\n").encode("ascii")
        if runner_sha_file_bytes != expected_runner_sha_text:
            raise ClassificationError("capture runner sha256 content mismatch")
        ...
        classification_record: dict[str, Any] = {
            ...
            "origin": "agy-" + ACTIVE_VERSION + "-models-capture",
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog=CLASSIFIER_PROG,
        description=f"Sidecar maintenance classifier for agy {ACTIVE_VERSION} models capture failure records.",
    )
    parser.add_argument(
        "--evidence-root",
        metavar="PATH",
        help=f"Explicit absolute path to owner-private {ACTIVE_VERSION} capture failure root.",
    )
    parser.add_argument(
        "--output",
        metavar="PATH",
...
def _bind_version_manifest(version_name: str, manifest_path: Optional[str] = None) -> None:
    global ACTIVE_VERSION, CLASSIFIER_PROG, EXPECTED_RUNNER_SHA256, RUNNER_ARTIFACT_NAME, EXPECTED_FILES
    global EXPECTED_SOURCE_SHA256, EXPECTED_RECOVERY_BINDING_SHA256
    global OUTPUT_PROFILE_NAME, RULESET_VERSION, RULESET_CANONICAL_BYTES, RULESET_SHA256
    ...
    values = version_manifest_engine.operation_constants(spec, "classifier")
    ACTIVE_VERSION = spec.version
    CLASSIFIER_PROG = "models_capture_" + spec.version.replace(".", "_") + "_classifier.py"
    RUNNER_ARTIFACT_NAME = "models_capture_" + spec.version.replace(".", "_") + "_runner.py"
    EXPECTED_RUNNER_SHA256 = values["EXPECTED_RUNNER_SHA256"]
    ...
    EXPECTED_FILES = frozenset({
        OUTPUT_PROFILE_NAME, RUNNER_ARTIFACT_NAME, RUNNER_ARTIFACT_NAME + ".sha256",
        "models.capture.failure.json", "models.stderr", "models.stdout",
    }) | SCRATCH_NAMES
    RULESET_CANONICAL_BYTES = _canonical({
        "categories": list(CATEGORIES[:-1]),
        "patterns": RULESET_PATTERNS,
...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 5 and sys.argv[3] == "--manifest":
        raise SystemExit(main_for_version(sys.argv[2], sys.argv[5:], sys.argv[4]))
    raise SystemExit(main_for_version(sys.argv[2], sys.argv[3:]))
