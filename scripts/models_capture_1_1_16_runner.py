#!/usr/bin/env python3
"""One separately-authorized, capture-only ``agy models`` observation for 1.1.16.
...
OUTPUT_PROFILE_NAME = "models.capture.1.1.16.profile.json"
...
MODULE_AST_SHA256 = "8951f45cd37b98b17c8c26579fbf837785aca18a9dc802bb555c783354a2d3b4"
...
def _validate_recovery(profile: Profile) -> None:
    ...
    try:
        ...
        if (value.get("claim") != "snapshot-version-only" or not isinstance(source, dict) or not isinstance(snapshot, dict) or not isinstance(version, dict)
                ...
                or version.get("logical_argv") != [profile.source_path, "--version"] or version.get("expected") != "1.1.16" or version.get("observed") != "1.1.16" or version.get("exit") != 0 or version.get("popen_count") != 1
...
def _new_root(parent: str, parent_fd: int, expected_parent: DirectoryIdentity) -> tuple[str, str, int, DirectoryIdentity]:
    ...
    for _ in range(32):
        name = "agy-models-capture-1-1-16." + os.urandom(12).hex()
        try:
...
def _verify_final_root(root_path: str, root_name: str, root_fd: int, root_identity: DirectoryIdentity, profile: Profile, capture_fd: int, artifacts: dict[str, tuple[str, FileIdentity]], scratch_ledger: dict[str, FileIdentity], marker: bool = False) -> None:
    expected = set(SCRATCH_NAMES) | {"models_capture_1_1_16_runner.py", "models_capture_1_1_16_runner.py.sha256", OUTPUT_PROFILE_NAME, "models.stdout", "models.stderr", "models.capture.summary.json", "models.capture.json"}
...
def run_capture(profile: Profile, profile_bytes: bytes, signals: Signals, runner_source: bytes) -> dict[str, object]:
    ...
    try:
        ...
        _write(root_fd, "models_capture_1_1_16_runner.py", runner_source, ledger=ledger)
        _write(root_fd, "models_capture_1_1_16_runner.py.sha256", (hashlib.sha256(runner_source).hexdigest() + "\n").encode("ascii"), ledger=ledger)
        ...
        artifacts = {OUTPUT_PROFILE_NAME: profile_sha, "models.capture.summary.json": summary_sha, "models.stderr": stderr_sha, "models.stdout": stdout_sha, "models_capture_1_1_16_runner.py": runner_sha, "models_capture_1_1_16_runner.py.sha256": hashlib.sha256((runner_sha + "\n").encode("ascii")).hexdigest()}
