#!/usr/bin/env python3
"""Create one fresh 1.1.22 source/snapshot binding for a later models capture.
...
observation, and writes only capture-source evidence.  It cannot run ``models``
and grants no provider, account, inventory, routing, or metadata authority.
"""
...
MODULE_AST_SHA256 = "bb5689dd825953aa4717a89316dbe282a176068f3c30938233db3aaec8a94d46"
...
EVIDENCE_FILES = frozenset(set(SCRATCH_NAMES) | {
    ...
    "snapshot.pre.json", "source.post.json", "source.pre.json",
    "version.binding.json", "version.binding.sha256", "version.stderr",
    "version.stdout", "version.summary.json",
})
...
def run_initial_bootstrap(profile: InitialProfile, *, lifecycle: Optional[LifecycleState] = None, process_owned: bool = False) -> dict[str, object]:
    ...
    complete = False
    try:
        runner_source = _module_source()
        runner_contract = validate_source_contract(runner_source)
        controller.poll()
        ...
        runner_sha = _publish(ledger, prior, "runner.py", runner_source, controller)
        if runner_sha != runner_contract["sha256"]:
            raise InitialBootstrapError("version-evidence runner copy changed")
        _publish(ledger, prior, "runner.py.sha256", (runner_sha + "\n").encode("ascii"), controller)
        ...
        ledger.inventory(prior, EVIDENCE_FILES)
        ...
        result = {"binding_sha256": binding_sha, "call_count": 1, "claim": "snapshot-version-capture-source", "initial_profile_sha256": initial_profile_sha, "profile": dataclasses.asdict(recovery_profile), "recovery_profile_sha256": recovery_profile_sha, "runner_sha256": runner_sha, "status": "accepted"}
        controller.poll()
        if process_owned:
