#!/usr/bin/env python3
"""One separately-authorized, capture-only ``agy models`` observation for 1.1.22.
...
MODULE_AST_SHA256 = "e4e5bf8ca0016b966ce4f8b5c551efd2d2dff74ea174c093f489774b74bb6253"
...
def _validate_recovery(profile: Profile) -> None:
    ...
    try:
        ...
        if (value.get("claim") != "snapshot-version-only" or not isinstance(source, dict) or not isinstance(snapshot, dict) or not isinstance(version, dict)
                ...
                or version.get("logical_argv") != [profile.source_path, "--version"] or version.get("expected") != "1.1.22" or version.get("observed") != "1.1.22" or version.get("exit") != 0 or version.get("popen_count") != 1
                or official != {"distribution_sha512": EXPECTED_DISTRIBUTION_SHA512, "distribution_url": EXPECTED_DISTRIBUTION_URL, "release_commit": EXPECTED_RELEASE_COMMIT, "version": "1.1.22"}
...
def _verify_final_root(root_path: str, root_name: str, root_fd: int, root_identity: DirectoryIdentity, profile: Profile, capture_fd: int, artifacts: dict[str, tuple[str, FileIdentity]], scratch_ledger: dict[str, FileIdentity], marker: bool = False, failure: bool = False) -> None:
    if failure:
        expected = set(SCRATCH_NAMES) | {"models_capture_1_1_22_runner.py", "models_capture_1_1_22_runner.py.sha256", OUTPUT_PROFILE_NAME, "models.stdout", "models.stderr", "models.capture.failure.json"}
    else:
        expected = set(SCRATCH_NAMES) | {"models_capture_1_1_22_runner.py", "models_capture_1_1_22_runner.py.sha256", OUTPUT_PROFILE_NAME, "models.stdout", "models.stderr", "models.capture.summary.json", "models.capture.json"}
...
def run_capture(profile: Profile, profile_bytes: bytes, signals: Signals, runner_source: bytes) -> dict[str, object]:
    ...
    try:
        ...
        if rc != 0:
            ...
            _write(root_fd, "models_capture_1_1_22_runner.py", runner_source, ledger=ledger)
            _write(root_fd, "models_capture_1_1_22_runner.py.sha256", (runner_sha + "\n").encode("ascii"), ledger=ledger)
        ...
        _write(root_fd, "models_capture_1_1_22_runner.py", runner_source, ledger=ledger)
        _write(root_fd, "models_capture_1_1_22_runner.py.sha256", (hashlib.sha256(runner_source).hexdigest() + "\n").encode("ascii"), ledger=ledger)
        _write(root_fd, OUTPUT_PROFILE_NAME, profile_bytes, ledger=ledger)
        ...
        runner_sha = hashlib.sha256(runner_source).hexdigest()
        artifacts = {OUTPUT_PROFILE_NAME: profile_sha, "models.capture.summary.json": summary_sha, "models.stderr": stderr_sha, "models.stdout": stdout_sha, "models_capture_1_1_22_runner.py": runner_sha, "models_capture_1_1_22_runner.py.sha256": hashlib.sha256((runner_sha + "\n").encode("ascii")).hexdigest()}
