#!/usr/bin/env python3
"""Process-inert reprofile preparation for a manifest-bound capture profile.
...
# ---------------------------------------------------------------------------
# Import the shared profile and runner modules for reuse of validation
# primitives. This entrypoint owns its publication implementation.
# ---------------------------------------------------------------------------

def _load_profile_module() -> object:
    """Load the shared profile implementation and bind it to the active version."""
    import importlib.util
    source_dir = os.path.dirname(os.path.abspath(__file__))
    profile_path = os.path.join(source_dir, "version_manifest_capture_profile.py")
    module_name = "_version_manifest_capture_profile"
    spec = importlib.util.spec_from_file_location(module_name, profile_path)
    if spec is None or spec.loader is None:
        raise ReprofileError("cannot locate shared capture profile module")
    mod = importlib.util.module_from_spec(spec)
    previous = sys.modules.get(module_name)
    sys.modules[module_name] = mod
    try:
        spec.loader.exec_module(mod)
        mod._bind_version_manifest(ACTIVE_VERSION)
        mod._held_source()
...
def _load_runner_module() -> object:
    """Load the shared runner implementation and bind it to the active version."""
    import importlib.util
    source_dir = os.path.dirname(os.path.abspath(__file__))
    runner_path = os.path.join(source_dir, "version_manifest_capture_runner.py")
    module_name = "_version_manifest_capture_runner"
    spec = importlib.util.spec_from_file_location(module_name, runner_path)
    if spec is None or spec.loader is None:
        raise ReprofileError("cannot locate shared capture runner module")
    mod = importlib.util.module_from_spec(spec)
    previous = sys.modules.get(module_name)
    sys.modules[module_name] = mod
    try:
...
def _bind_version_manifest(version_name: str, manifest_path: Optional[str] = None) -> None:
    global ACTIVE_VERSION, OUTPUT_NAME, _profile_mod, _runner_mod
    ...
    values = version_manifest_engine.operation_constants(spec, "reprofile")
