#!/usr/bin/env python3
"""Offline structural controls for the separate 1.1.22 models-capture bridge.
...
def profile_value() -> dict[str, object]:
    return {
        ...
        "snapshot_identity": identity(), "snapshot_path": "/private/capture/recovery/agy.snapshot",
...
class ModelsCapture1122Tests(unittest.TestCase):
    def _synthetic_runner_profile(self, base: str, exit_code: int, scratch_write: bool = False, tmp_cache: str = "", tmp_cache_mode: str = "", child_sets_private_umask: bool = True, snapshot_policy: str = "stable", updater_attempt: bool = False) -> tuple[object, bytes, dict[str, object], dict[str, object]]:
        ...
        summary = b"{}\n"; recovery_runner = b"r"
        artifacts = {"runner.py": hashlib.sha256(recovery_runner).hexdigest(), "version.summary.json": hashlib.sha256(summary).hexdigest(), "version.stdout": hashlib.sha256(b"1.1.22\n").hexdigest(), "version.stderr": hashlib.sha256(b"").hexdigest()}
        binding_value = {"artifacts": artifacts, "claim": "snapshot-version-only", "inventory": {"executable_version_bound": False}, "limitations": {"account_read": False, "metadata_advance_authorized": False, "models_called": False, "network_absence_os_enforced": False, "provider_backend_proven": False, "routing_authority": False}, "official_observation": {"distribution_sha512": runner["EXPECTED_DISTRIBUTION_SHA512"], "distribution_url": runner["EXPECTED_DISTRIBUTION_URL"], "release_commit": runner["EXPECTED_RELEASE_COMMIT"], "version": "1.1.22"}, "source": {"pre": runner["dataclasses"].asdict(source_identity), "post": runner["dataclasses"].asdict(source_identity), "sha256": source_sha}, "snapshot": {"pre": runner["dataclasses"].asdict(snapshot_identity), "post": runner["dataclasses"].asdict(snapshot_identity), "sha256": source_sha}, "version": {"exit": 0, "expected": "1.1.22", "logical_argv": [source, "--version"], "observed": "1.1.22", "popen_count": 1}}
        ...
        files = {"runner.py": recovery_runner, "runner.py.sha256": (runner_sha + "\n").encode("ascii"), "source.pre.json": canonical(runner["dataclasses"].asdict(source_identity)), "source.post.json": canonical(runner["dataclasses"].asdict(source_identity)), "snapshot.pre.json": canonical(runner["dataclasses"].asdict(snapshot_identity)), "snapshot.post.json": canonical(runner["dataclasses"].asdict(snapshot_identity)), "version.binding.json": binding, "version.binding.sha256": (binding_sha + "\n").encode("ascii"), "version.stderr": b"", "version.stdout": b"1.1.22\n", "version.summary.json": summary}
    ...
    def test_01_profile_source_contract(self) -> None:
        ...
        self.assertEqual(profile["EXPECTED_RECOVERY_BINDING_SHA256"], "d9d830e65d3a5c76df6d9e07e6ea7e14e14f290ab4036bdbae8cb33502e29f2a")
        self.assertEqual(profile["EXPECTED_RECOVERY_RUNNER_SHA256"], "e2f6a50cad78ebf572719d81a5d1d5fee40b31808d960a0ac3f800db2bf9b9b7")
        self.assertEqual(profile["EXPECTED_DISTRIBUTION_URL"], "https://storage.googleapis.com/antigravity-public/antigravity-cli/1.1.22-5711547746615296/darwin-arm/cli_mac_arm64.tar.gz")
        self.assertEqual(profile["EXPECTED_RECOVERY_RUNNER_BYTES"], 46_027)
        self.assertEqual(profile["EXPECTED_RECOVERY_SUMMARY_BYTES"], 260)
    ...
    def test_02_runner_source_contract(self) -> None:
        ...
        self.assertEqual(runner["EXPECTED_RECOVERY_BINDING_SHA256"], "d9d830e65d3a5c76df6d9e07e6ea7e14e14f290ab4036bdbae8cb33502e29f2a")
        self.assertEqual(runner["EXPECTED_RECOVERY_RUNNER_SHA256"], "e2f6a50cad78ebf572719d81a5d1d5fee40b31808d960a0ac3f800db2bf9b9b7")
        self.assertEqual(runner["EXPECTED_DISTRIBUTION_URL"], "https://storage.googleapis.com/antigravity-public/antigravity-cli/1.1.22-5711547746615296/darwin-arm/cli_mac_arm64.tar.gz")
        self.assertEqual(runner["EXPECTED_RECOVERY_RUNNER_BYTES"], 46_027)
        self.assertEqual(runner["EXPECTED_RECOVERY_SUMMARY_BYTES"], 260)
    ...
    def test_11_no_legacy_runner_import(self) -> None:
        ...
        self.assertNotIn("version_recovery_1_1_22_runner", text)
    ...
    def test_17_capture_is_not_acceptance(self) -> None:
        ...
        self.assertIn('"inventory_interpreted": False', source)
        self.assertIn('"metadata_advance_authorized": False', source)
        self.assertIn('"metadata_updated": False', source)
        self.assertIn('"routing_authorized": False', source)
    ...
    def test_19_recovery_inventory_is_18_members(self) -> None:
        self.assertEqual(len(runner["RECOVERY_FILES"]), 18)
    ...
    def test_22_profile_rejects_wrong_output_basename_before_authority(self) -> None:
        value = {"account_home": "/private/account", "capture_parent": "/private/capture", "output_path": "/private/capture/other.json", "snapshot_path": "/private/capture/recovery/agy.snapshot", "source_path": "/private/capture/agy.source", "version_root": "/private/capture/recovery"}
    ...
    def test_27_group_uses_logical_source_and_snapshot_executable(self) -> None:
        source = RUNNER_SOURCE.read_text()
        self.assertIn("executable=profile.snapshot_path", source)
        self.assertIn("pgid != process.pid", source)
        self.assertIn("_close_group(process, reserved_pgid)", source)
    ...
    def test_47_start_capture_uses_exact_popen_contract(self) -> None:
        ...
        try:
            ...
            process, reserved_pgid = runner["_start_capture"](value, "/private/capture/root")
        finally:
    ...
    def test_52_full_fake_capture_publishes_nonsemantic_record_and_marker(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            globals_["_start_capture"] = lambda *_args: (FakeProcess(), FakeProcess.pid)
            globals_["_capture"] = lambda *_args: (b"captured output\n", b"")
            globals_["_close_group"] = lambda _process, _reserved_pgid: 0
            try:
    ...
    def test_56_real_fake_child_success_uses_runner_subprocess_and_publishes(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(output, [paths["snapshot"], "--output-format", paths["account"], os.path.join(root, "tmp"), os.path.join(root, "xdg-cache"), "/usr/bin:/bin", "C"])
            self.assertTrue(os.path.exists(os.path.join(root, "models.capture.sha256")))

    def test_57_real_fake_child_nonzero_preserves_private_failure_evidence_without_marker(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertFalse(os.path.exists(os.path.join(root, "models.capture.sha256")))
            expected = set(runner["SCRATCH_NAMES"]) | {
                runner["OUTPUT_PROFILE_NAME"], "models.stdout", "models.stderr",
                "models.capture.failure.json", "models_capture_1_1_22_runner.py",
                "models_capture_1_1_22_runner.py.sha256",
            }
            self.assertEqual(set(os.listdir(root)), expected)
            with open(os.path.join(root, "models.capture.failure.json"), encoding="ascii") as handle:
                failure = json.load(handle)
            self.assertEqual(failure["status"], "child-failed")
            self.assertEqual(failure["observation"], {"exit": 7, "popen_count": 1})
            self.assertFalse(failure["limitations"]["failure_classified"])
            self.assertNotIn("account_home", failure)
            with open(os.path.join(root, "models.stdout"), "rb") as handle:
                self.assertTrue(handle.read())
            with open(os.path.join(root, "models.stderr"), "rb") as handle:
                self.assertEqual(handle.read(), b"")
            for name in expected - set(runner["SCRATCH_NAMES"]):
                self.assertEqual(os.stat(os.path.join(root, name)).st_mode & 0o777, 0o600)
    ...
    def test_82_exit_zero_child_scratch_write_rejects_without_marker_and_preserves_residual(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for name in (runner["OUTPUT_PROFILE_NAME"], "models.stdout", "models.stderr", "models.capture.summary.json", "models.capture.json", "models.capture.sha256", "models_capture_1_1_22_runner.py", "models_capture_1_1_22_runner.py.sha256"):
    ...
    def test_85_unknown_tmp_cache_rejects_and_preserves_residual(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            base = os.path.realpath(temporary)
            value, raw, old, paths = self._synthetic_runner_profile(base, 0, tmp_cache="other-cache.json")
            globals_ = runner["run_capture"].__globals__
            try:
                with self.assertRaises(runner["CaptureError"]): runner["run_capture"](value, raw, PollCounter(), RUNNER_SOURCE.read_bytes())
            finally:
            ...
            roots = [entry for entry in os.listdir(paths["parent"]) if entry.startswith("agy-models-capture-1-1-22.")]
            self.assertEqual(len(roots), 1)
            self.assertTrue(os.path.isfile(os.path.join(paths["parent"], roots[0], "tmp", "other-cache.json")))

    def test_86_wrong_mode_known_tmp_cache_rejects_and_preserves_residual(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            base = os.path.realpath(temporary)
            value, raw, old, paths = self._synthetic_runner_profile(base, 0, tmp_cache=runner["TMP_CACHE_LEAF"], tmp_cache_mode="0644")
            globals_ = runner["run_capture"].__globals__
            try:
                with self.assertRaises(runner["CaptureError"]): runner["run_capture"](value, raw, PollCounter(), RUNNER_SOURCE.read_bytes())
            finally:
            ...
            roots = [entry for entry in os.listdir(paths["parent"]) if entry.startswith("agy-models-capture-1-1-22.")]
            self.assertEqual(len(roots), 1)
            cache = os.path.join(paths["parent"], roots[0], "tmp", runner["TMP_CACHE_LEAF"])
            self.assertEqual(os.stat(cache).st_mode & 0o777, 0o644)

    def test_87_registered_group_closes_surviving_descendant_before_leader_reap(self) -> None:
        """Leader exit/closed stdio cannot bypass the already reserved PGID."""
        events: list[tuple[str, int]] = []
        class ExitedLeader:
            pid = 9187
            returncode = None  # exited but intentionally not polled or reaped
            waited = False
            def wait(self, timeout: float) -> int:
                self.waited = True
                events.append(("wait", int(timeout * 1000)))
                self.returncode = 0
                return 0
        process = ExitedLeader()
        globals_ = runner["_close_group"].__globals__
        original_killpg, original_getpgid = globals_["os"].killpg, globals_["os"].getpgid
        zero_probes = ["descendant-survives", "group-gone"]
        def killpg(pgid: int, signum: int) -> None:
            self.assertFalse(process.waited, "reserved PGID was used after leader reap")
            self.assertEqual(pgid, process.pid)
            events.append(("signal", signum))
            if signum == 0:
                state = zero_probes.pop(0)
                if state == "group-gone":
                    raise ProcessLookupError
        globals_["os"].killpg = killpg
        globals_["os"].getpgid = lambda _pid: (_ for _ in ()).throw(AssertionError("registered group must not be rediscovered"))
        try:
            self.assertEqual(runner["_close_group"](process, process.pid), 0)
        finally:
            globals_["os"].killpg, globals_["os"].getpgid = original_killpg, original_getpgid
        self.assertEqual(events[0], ("signal", runner["signal"].SIGTERM))
        self.assertIn(("signal", 0), events[:-1])
        self.assertEqual(events[-1][0], "wait")
        self.assertEqual(zero_probes, [])

    def test_88_registered_group_kill_deadline_fails_closed_before_reap(self) -> None:
        events: list[str] = []
        class ExitedLeader:
            pid = 9188
            returncode = None
        ...
        process = ExitedLeader()
        globals_ = runner["_close_group"].__globals__
        original_killpg, original_monotonic, original_sleep = globals_["os"].killpg, globals_["time"].monotonic, globals_["time"].sleep
        moments = iter((0.0, 1.0))
        def killpg(_pgid: int, _signum: int) -> None:
            self.assertNotIn("wait", events, "group operation occurred after leader reap")
            events.append("group")
        globals_["os"].killpg = killpg
        globals_["time"].monotonic = lambda: next(moments)
        globals_["time"].sleep = lambda _seconds: None
        try:
            with self.assertRaises(runner["CaptureError"]) as raised:
                runner["_close_group"](process, process.pid)
        finally:
            globals_["os"].killpg, globals_["time"].monotonic, globals_["time"].sleep = original_killpg, original_monotonic, original_sleep
