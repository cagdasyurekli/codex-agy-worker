#!/usr/bin/env python3
"""Offline capture-runner slice of the 1.1.22 capture bridge tests."""
...
SOURCE = pathlib.Path(__file__).with_name("test-models-capture-1-1-22.py")
...
if __name__ == "__main__":
    all_tests = unittest.defaultTestLoader.getTestCaseNames(MODULE["ModelsCapture1122Tests"])
    suite = unittest.TestSuite(
        MODULE["ModelsCapture1122Tests"](name) for name in all_tests if name not in PROFILE_TESTS
    )
