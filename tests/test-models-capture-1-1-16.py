#!/usr/bin/env python3
"""Offline structural controls for the separate 1.1.16 models-capture bridge.
...
PROFILE_SOURCE = ROOT / "scripts" / "models_capture_1_1_16_profile.py"
RUNNER_SOURCE = ROOT / "scripts" / "models_capture_1_1_16_runner.py"
...
class ModelsCapture116Tests(unittest.TestCase):
    def _synthetic_runner_profile(self, base: str, exit_code: int, scratch_write: bool = False, tmp_cache: str = "", tmp_cache_mode: str = "", child_sets_private_umask: bool = True) -> tuple[object, bytes, dict[str, object], dict[str, object]]:
        ...
        summary = b"{}\n"; recovery_runner = b"r"
        artifacts = {"runner.py": hashlib.sha256(recovery_runner).hexdigest(), "version.summary.json": hashlib.sha256(summary).hexdigest(), "version.stdout": hashlib.sha256(b"1.1.16\n").hexdigest(), "version.stderr": hashlib.sha256(b"").hexdigest()}
        binding_value = {"artifacts": artifacts, "claim": "snapshot-version-only", "inventory": {"executable_version_bound": False}, "limitations": {"account_read": False, "metadata_advance_authorized": False, "models_called": False, "network_absence_os_enforced": False, "provider_backend_proven": False, "routing_authority": False}, "official_observation": {"distribution_sha512": runner["EXPECTED_DISTRIBUTION_SHA512"], "distribution_url": runner["EXPECTED_DISTRIBUTION_URL"], "release_commit": runner["EXPECTED_RELEASE_COMMIT"], "version": "1.1.16"}, "source": {"pre": runner["dataclasses"].asdict(source_identity), "post": runner["dataclasses"].asdict(source_identity), "sha256": source_sha}, "snapshot": {"pre": runner["dataclasses"].asdict(snapshot_identity), "post": runner["dataclasses"].asdict(snapshot_identity), "sha256": source_sha}, "version": {"exit": 0, "expected": "1.1.16", "logical_argv": [source, "--version"], "observed": "1.1.16", "popen_count": 1}}
    ...
    def test_01_profile_source_contract(self) -> None:
        ...
        self.assertEqual(profile["EXPECTED_RECOVERY_BINDING_SHA256"], "facf6adc18afc85ed5c232e3e1f9ad0fbcac7d62f1f98866cabb615d43069a57")
        self.assertEqual(profile["EXPECTED_RECOVERY_RUNNER_SHA256"], "9c1a9d35c0db9fe137ed4490b47d2b11443fe7cfaf1e552eca7adf575b048d4c")
    ...
    def test_02_runner_source_contract(self) -> None:
        ...
        self.assertEqual(runner["EXPECTED_RECOVERY_BINDING_SHA256"], "facf6adc18afc85ed5c232e3e1f9ad0fbcac7d62f1f98866cabb615d43069a57")
        self.assertEqual(runner["EXPECTED_RECOVERY_RUNNER_SHA256"], "9c1a9d35c0db9fe137ed4490b47d2b11443fe7cfaf1e552eca7adf575b048d4c")
    ...
    def test_11_no_legacy_runner_import(self) -> None:
        ...
        self.assertNotIn("version_recovery_1_1_16_runner", text)
    ...
    def test_57_real_fake_child_nonzero_rejects_without_marker(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            try:
                ...
                globals_.update(old)
            roots = [entry for entry in os.listdir(paths["parent"]) if entry.startswith("agy-models-capture-1-1-16.")]
    ...
    def test_82_exit_zero_child_scratch_write_rejects_without_marker_and_preserves_residual(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for name in (runner["OUTPUT_PROFILE_NAME"], "models.stdout", "models.stderr", "models.capture.summary.json", "models.capture.json", "models.capture.sha256", "models_capture_1_1_16_runner.py", "models_capture_1_1_16_runner.py.sha256"):
