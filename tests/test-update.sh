...
git -C "$UPSTREAM_SOURCE" tag v1.2.11
...
printf '%s\n' "${FAKE_AGY_OUTPUT:-${FAKE_AGY_VERSION:-1.2.11}}"
STUB
    ...
    case "$profile" in
      ...
      official-agy)
        case "${FAKE_AGY_OFFICIAL_MODE:-unchanged}" in
          ...
          *) printf 'agy\t%s\t%s\n' "${FAKE_AGY_OFFICIAL_VERSION:-1.2.11}" "${FAKE_AGY_OFFICIAL_HEAD:-$FAKE_AGY_HEAD}" ;;
        esac
        ;;
      official-codex)
...
' "$1"; then
    ...
    case "${FAKE_MANIFEST_RESULT:-unchanged}" in
      unchanged)
        printf '%s\n' '  distribution manifest: unchanged (1.2.11)'
        exit 0 ;;
      drift)
        printf '%s\n' '  distribution manifest: drift-review (official distribution 1.2.12; verified 1.2.11)'
        exit 3 ;;
      unavailable)
...
export FAKE_AGY_HEAD="6dadd6227a49905f475d22b7f0afe59493229595"
