#!/usr/bin/env python3
"""Offline capture-runner slice of the 1.1.16 capture bridge tests."""
...
SOURCE = pathlib.Path(__file__).with_name("test-models-capture-1-1-16.py")
...
if __name__ == "__main__":
    all_tests = unittest.defaultTestLoader.getTestCaseNames(MODULE["ModelsCapture116Tests"])
    suite = unittest.TestSuite(
        MODULE["ModelsCapture116Tests"](name) for name in all_tests if name not in PROFILE_TESTS
    )
