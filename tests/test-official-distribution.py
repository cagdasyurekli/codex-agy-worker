...
ARCHIVE_URL = (
    ...
    "1.1.27-5211191891591168/darwin-arm/cli_mac_arm64.tar.gz"
)
...
def _() -> None:
    expect_error(
        "invalid archive policy",
        lambda: parse(url=replace_url("1.1.27-", "1.1.9-")),
    )
...
def _() -> None:
    expect_error(
        "invalid archive policy",
        lambda: parse(url=replace_url("5211191891591168", "build-secret")),
    )
...
def _() -> None:
    ...
    assert status == 3 and "1.1.27" in detail and "1.1.9" in detail


@test("same-version archive build drift is drift-review")
def _() -> None:
    observed = parse()
    snapshot = parse(url=replace_url("5211191891591168", "6607970839166975"))
...
@test("observed 1.1.27 against a historical 1.1.16 snapshot is drift-review")
def _() -> None:
    with tempfile.TemporaryDirectory() as temporary:
        root = Path(temporary)
        (root / "compat").mkdir()
        ...
        assert status == 3 and "1.1.27" in detail and "1.1.16" in detail
        assert [call[0].full_url for call in opener.calls] == [distribution.MANIFEST_URL]
