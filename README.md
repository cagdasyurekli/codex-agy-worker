...
These are existing wrapper constants, not values generated by the compatibility
matrix. Activating that reviewed metadata does not change a tier or add a new input.
Any other tier value is passed through as an explicit agy model label. agy has a real
`--effort`, but this wrapper has neither an effort input nor a separate thinking-level
control. Thinking/effort-labelled compound slugs remain exact selected model labels.
...
separately approved G1. The checked-in active model/effort matrix is validated
compatibility metadata, not routing or gate authority. It maps eight explicit
...
and reviewed source revision match the canonical records. The wrapper does not yet
consume this matrix or send agy's separate effort flag. `qa-gate.sh` remains the sole
...
tests/test-update.sh          174-case offline local-remote/matrix/manifest updater suite
...
tests/test-packaging.sh       89-case offline Codex package/relocation/landing suite
tests/test-doctor.sh          148-case offline fake-tool/read-only doctor suite
...
- Model/effort resolution is active only as version/source-bound compatibility
  metadata. No wrapper `--model`/`--effort` input consumes it in this release; that
  remains a separately approved G1 slice.
